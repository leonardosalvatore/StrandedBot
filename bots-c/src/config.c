...
static void apply_fallback_defaults(BotsConfig *out) {
    ...
    /* Single global preset (formerly the explorer numbers). */
    out->game.rocks_amount            = 20;
    out->game.initial_town_size       = 0;
    out->game.energy                  = 1000;
    out->game.inventory_rocks         = 10;
    out->game.hours_solar_flare_every = 100;
...
static bool overlay_json(BotsConfig *out, const char *json_text,
    ...
    if (cJSON_IsObject(game)) {
        copy_json_int(game, "rocks_amount",            &out->game.rocks_amount);
        copy_json_int(game, "initial_town_size",       &out->game.initial_town_size);
        copy_json_int(game, "energy",                  &out->game.energy);
        copy_json_int(game, "inventory_rocks",         &out->game.inventory_rocks);
        copy_json_int(game, "hours_solar_flare_every", &out->game.hours_solar_flare_every);
    }
...
static cJSON *config_to_json(const BotsConfig *cfg) {
    ...
    cJSON_AddNumberToObject(game, "rocks_amount",            cfg->game.rocks_amount);
    cJSON_AddNumberToObject(game, "initial_town_size",       cfg->game.initial_town_size);
    cJSON_AddNumberToObject(game, "energy",                  cfg->game.energy);
    cJSON_AddNumberToObject(game, "inventory_rocks",         cfg->game.inventory_rocks);
    cJSON_AddNumberToObject(game, "hours_solar_flare_every", cfg->game.hours_solar_flare_every);
